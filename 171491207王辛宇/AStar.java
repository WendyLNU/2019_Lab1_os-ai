	...
	/** 
	   ...
	   */
		...
		public void inOpen(Node node,Map map){
			...
			for (int i = 0;i<3;i++){
				for (int j = 0;j<3;j++){
					...
					if(map.getMap()[x-1+i][y-1+j].isReachable()&&!open.contains(map.getMap()[x-1+i][y-1+j])&&
							!(x==(x-1+i)&&y==(y-1+j))){
		...
		public void search(Map map){
			//对起点及起点周围的节点进行操作 
			inOpen(map.getMap()[map.getStartNode().getX()][map.getStartNode().getY()],map);//那一长串是起点A
			close.add(map.getMap()[map.getStartNode().getX()][map.getStartNode().getY()]);//将起点A加入close表
			...
			//知道开启列表中包含终点B时，退出循环
			inClose(map.getMap()[map.getEndNode().getX()][map.getEndNode().getY()], open);//将结束点放入close表
			showPath(close,map);
		}
		//将路径表示出来的函数
